...
class XUpload extends CJuiInputWidget {
    ...
    public $bootsrapPath = '/css/bootstrap/';
    ...
    public function publishAssets() {
        ...
        if ($this->registerBootstrap) {
            if ($this->bootsrapPath) {
                $bootstrapBaseUrl = CHtml::asset( Yii::getPathOfAlias('webroot').$this->bootsrapPath );
                Yii::app()->clientScript->registerCssFile($bootstrapBaseUrl . '/css/bootstrap.min.css');
                Yii::app()->clientScript->registerScriptFile($bootstrapBaseUrl . '/js/bootstrap.min.js', CClientScript::POS_END);
            } else {
                throw new CHttpException(500, __CLASS__ . ' - Error: Couldn\'t find assets to publish.');
            }
        }
